...
alter table Tecnico rename column dataAdimissao to dataAdmissao;
...
create table Finalizacao(
	idFinalizaco int not null auto_increment primary key,
...
alter table Finalizacao rename column idFinalizaco to idFinalizacao;
...
create table OrdemServico(
	...
	estado enum('Aberta', 'Em execução', 'Concluida') not null,
	...
	constraint fk_OrdemServico_Finalizacao
		FOREIGN KEY (idFinalizacao) REFERENCES Finalizacao(idFinalizacao));

alter table OrdemServico modify idFinalizacao int;
...
-- 2 CONSULTAS
/* 1 consulta para listar o total/quantidade de serviços cadastrados */
select count(idServico) as "Total de serviços cadastrados"
from Servico;

/* 2 consulta para listar a data de abertura, o equipamento e aprioridade de todas as ordens de serviço cujo estado seja “Aberta”. 
...
/* 3 consulta para listar os serviços executados em cada ordem deserviço. A lista deve retornar à identificação da ordem de serviço, 
...
/* 4 consulta para listar a identificação de cada ordem de serviço e seu total, isto é, a soma de todos os subtotais de seus serviços executados, 
